    ...
    <>
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          ...
          <p className="text-base text-slate-600 dark:text-white/70 mt-2">Registra clientes, gestiona sus datos y vincula propiedades a su perfil.</p>
        </div>
      ...
      <div className="grid grid-cols-1 mb-8">
        <div className="bg-white dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/10 p-6 shadow-sm">
          <h4 className="text-[12px] font-bold text-slate-500 dark:text-white/60 uppercase tracking-wider">Clientes Totales</h4>
          <div className="text-[24px] font-bold text-build-main dark:text-white mt-1">
      ...
      {/* Toolbar */}
      <div className="mb-4 flex flex-col md:flex-row gap-4 justify-between items-center bg-white dark:bg-white/5 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-white/10">
        <form onSubmit={handleSearch} className="flex gap-2 w-full md:w-1/3">
          <input
            ...
            className="w-full px-4 py-2 border border-slate-200 dark:border-white/10 rounded-lg text-sm focus:outline-none focus:border-build-main"
          />
          <button type="submit" className="px-4 py-2 bg-slate-100 dark:bg-white/10 text-build-main dark:text-white rounded-lg font-bold text-sm hover:bg-slate-200 dark:bg-white/15 transition-colors">
            Buscar
          </button>
        </form>
            ...
            <tbody className="text-sm text-build-main dark:text-white">
              ...
              {!listLoading && clients.map((c) => (
                <tr 
                  ...
                  className="border-b border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:bg-white/5 transition-colors group cursor-pointer"
                >
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3 text-left">
                      <div className="w-9 h-9 rounded-full bg-build-bg flex items-center justify-center text-build-main dark:text-white text-xs font-bold">{c.initials}</div>
                      <div>
                        <span className="font-semibold text-build-main dark:text-white">{c.name}</span>
                  ...
                  <td className="py-4 px-6 font-medium text-build-main dark:text-white">{c.project}</td>
                  <td className="py-4 px-6">
                  ...
                  <td className="py-4 px-6 text-right">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="text-build-main dark:text-white hover:bg-[#c2e8ff] p-1.5 rounded-md transition-colors"
                        title="Editar perfil"
                      >
      ...
      {revokeOpen && clientToRevoke && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#050a0e]/60 backdrop-blur-sm animate-fade-in p-4">
          <div className="bg-white dark:bg-white/5 rounded-2xl shadow-2xl w-full max-w-lg p-8 relative overflow-hidden">
            ...
            <div className="flex flex-col items-center text-center">
              <div className="w-16 h-16 rounded-full bg-[#ffdad6] flex items-center justify-center mb-5 border border-[#ba1a1a]/10 relative z-10 shadow-[0_0_20px_rgba(186,26,26,0.1)]">
                ...
                <div className="absolute -bottom-1 -right-1 bg-white dark:bg-white/5 rounded-full p-0.5">
                  <span className="material-symbols-outlined text-build-main dark:text-white text-[16px]">close</span>
                </div>
              </div>

              <h3 className="text-[22px] font-bold text-build-main dark:text-white mb-2 leading-tight">Desvincular propiedad</h3>
              ...
              {!successMsg && (
                <div className="w-full text-left mb-8 animate-fade-in">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-build-accent rounded-lg text-sm font-medium focus:outline-none focus:border-[#ba1a1a] hover:border-[#ba1a1a]/50 bg-white dark:bg-white/5"
                  >
                  ...
                  <div className="flex gap-3 items-start mt-5 p-3 rounded-xl bg-build-bg border border-build-accent">
                    ...
                    <p className="text-[12px] text-build-accent leading-relaxed">
                      ...
                      revocará en tiempo real el <span className="font-semibold text-build-main dark:text-white">acceso a los Documentos</span> y lo
