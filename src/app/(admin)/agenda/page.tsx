  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold tracking-[-0.01em] text-build-main">Agenda y Citas</h2>
          <p className="text-base text-slate-600 mt-2">Programa reuniones, firmas, entregas y eventos importantes con clientes.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Calendar Side */}
        <div className="lg:col-span-9 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col min-h-[600px] animate-fade-in">
          <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-white">
            <div className="flex items-center gap-4">
              <h3 className="text-[20px] font-bold text-build-main capitalize">{new Date().toLocaleString('es-ES', { month: 'long', year: 'numeric' })}</h3>
              <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-0.5">
                <button className="p-1 rounded hover:bg-slate-50 text-slate-500 transition-colors"><span className="material-symbols-outlined text-[18px]">chevron_left</span></button>
                <button className="px-3 py-1 text-[12px] font-bold text-build-main hover:bg-slate-50 rounded transition-colors">Hoy</button>
                <button className="p-1 rounded hover:bg-slate-50 text-slate-500 transition-colors"><span className="material-symbols-outlined text-[18px]">chevron_right</span></button>
              </div>
            </div>
            <div className="flex bg-white border border-slate-200 rounded-lg p-0.5">
              <button className="px-4 py-1.5 text-[12px] font-bold bg-slate-100 text-build-main rounded-md transition-all shadow-sm">Mes</button>
              <button className="px-4 py-1.5 text-[12px] font-bold text-slate-500 hover:text-build-main rounded-md transition-all">Semana</button>
            </div>
          </div>

          <div className="flex-1 grid grid-cols-7 border-l border-slate-200">
            {["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"].map(d => (
              <div key={d} className="py-3 text-center border-r border-b border-slate-200 bg-slate-50">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{d}</span>
              </div>
            ))}
            {calDays.map((cell, idx) => (
              <div key={idx} className={`min-h-[120px] border-b border-r border-slate-200 p-2 flex flex-col gap-1 ${cell.grey ? "bg-slate-50/50" : "cursor-pointer hover:bg-slate-50 transition-colors"}`}>
                <span className={`text-sm pl-1 mb-1 ${cell.today ? "w-7 h-7 flex items-center justify-center bg-build-main text-white rounded-full font-bold shadow-sm" : cell.grey ? "text-slate-400" : "text-build-main font-bold"}`}>
                  {cell.day}
                </span>
                {cell.events.map((ev, i) => (
                  <div key={i} className={`${ev.bg} rounded px-2 py-1.5 flex items-center gap-1.5 shadow-sm border border-build-main/5`}>
                    {ev.icon
        ...
        <div className="lg:col-span-3 flex flex-col gap-6">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            ...
            <h3 className="text-[18px] font-bold text-build-main">Estado de sincronización</h3>
              <span className="material-symbols-outlined text-slate-500">science</span>
            </div>

            <label className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-100 transition-colors">
              ...
              <div>
                <p className="text-[13px] font-bold text-build-main leading-tight">Simular fallo de Google Calendar</p>
                <p className="text-[11px] text-slate-500 mt-1 pr-2">Si ocurre un fallo, la cita igual se guarda y se muestra una advertencia de reintento automático.</p>
              </div>
            </label>
          </div>

          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <h3 className="text-[18px] font-bold text-build-main mb-4">Próximos Eventos</h3>
            {calDays.filter(d => !d.grey).flatMap(d => d.events).slice(0, 3).map(ev => (
              <div key={ev.id} className="mb-4 last:mb-0">
                <h4 className="text-[13px] font-bold text-build-main truncate">{ev.label}</h4>
            ...
            {!successMsg && !warningMsg && (
              <>
                <div className="px-6 py-5 border-b border-slate-200 bg-white flex justify-between items-center">
                  <h2 className="text-[20px] font-bold text-build-main flex items-center gap-2">
                    <span className="material-symbols-outlined text-build-accent">edit_calendar</span> Agendar cita
                  </h2>
                  <button type="button" onClick={() => setModalOpen(false)} disabled={isSaving} className="text-slate-400 hover:text-[#ba1a1a]">
                ...
                <div className="p-6 overflow-y-auto max-h-[70vh]">
                  ...
                  <div className="mb-4">
                    <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Cliente y Unidad *</label>
                    <select
                      value={clientId}
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="col-span-2">
                      <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Día Protocolar *</label>
                      <input type="date" value={eventDate} onChange={e => setEventDate(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-build-main focus:outline-none focus:border-build-accent focus:ring-1 focus:ring-build-accent" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Hora Inicio *</label>
                      <input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-build-main focus:outline-none focus:border-build-accent focus:ring-1 focus:ring-build-accent" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Hora Fin *</label>
                      <input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-build-main focus:outline-none focus:border-build-accent focus:ring-1 focus:ring-build-accent" />
                    </div>
                  </div>

                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Lugar / Ubicación</label>
                    <input type="text" value={location} onChange={e => setLocation(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-build-main focus:outline-none focus:border-build-accent focus:ring-1 focus:ring-build-accent" />
                  </div>
                </div>

                <div className="px-6 py-4 flex justify-end gap-3 bg-white border-t border-slate-200">
                  <button type="button" onClick={() => setModalOpen(false)} disabled={isSaving} className="px-5 py-2.5 text-sm font-bold text-slate-500 hover:bg-slate-50 hover:text-build-main transition-colors rounded-xl">
                    Cancelar
                  </button>
