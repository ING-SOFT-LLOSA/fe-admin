...
export default function ViewClienteModal({ open, cliente, onClose }: ViewClienteModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#050a0e]/50 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-white dark:bg-white/5 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-slide-up relative">
        {/* Header decoration */}
        <div className="h-24 bg-build-main relative">
          <div className="absolute -bottom-8 left-8 w-16 h-16 rounded-full bg-build-bg border-4 border-white flex items-center justify-center text-build-main dark:text-white text-xl font-bold shadow-sm">
            {cliente.initials}
          </div>
          <button
            onClick={onClose}
        ...
        <div className="pt-10 px-8 pb-8">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-[20px] font-bold text-build-main dark:text-white">{cliente.name}</h2>
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-[12px] text-slate-500 dark:text-white/60 font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-4">
            <div>
              ...
              <div className="text-sm font-semibold text-build-main dark:text-white flex items-center gap-2">
          ...
          <div className="mt-8 flex justify-end">
            <button
              onClick={onClose}
              className="px-6 py-2.5 bg-slate-100 dark:bg-white/10 text-build-main dark:text-white rounded-xl text-sm font-bold hover:bg-slate-200 dark:bg-white/15 transition-colors"
            >
              Cerrar
            </button>
          </div>
